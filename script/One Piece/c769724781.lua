...
function s.initial_effect(c)
	...
	Link.AddProcedure(c,aux.NOT(aux.FilterBoolFunctionEx(Card.IsType,TYPE_TOKEN)),3)
...
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(s.desfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,c,c,tp) 
		and Duel.IsPlayerCanSpecialSummonMonster(tp,926540951,0x135,TYPES_TOKEN,0,0,1,RACE_AQUA,ATTRIBUTE_LIGHT) end
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and Duel.Destroy(tc,REASON_EFFECT)>0
		...
		and Duel.IsPlayerCanSpecialSummonMonster(tp,926540951,0x135,TYPES_TOKEN,0,0,1,RACE_AQUA,ATTRIBUTE_LIGHT) then
		local token=Duel.CreateToken(tp,926540951)
